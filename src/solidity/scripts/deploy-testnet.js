  ...
  .setAction(async function (taskArguments, hre) {
    ...
    if (!(chainId in availableChains)) {
      ...
      Object.values(availableChains).forEach((chain) => {
        chainsForPrinting[chain.CHAIN_ID] = `${chain.CHAIN_NAME} (${chain.NETWORK_NAME})`;
      });
    ...
    console.log(
      '\x1b[32m%s\x1b[0m',
      `NFT deployed to ${chainConfig.CHAIN_NAME} (${chainConfig.NETWORK_NAME}): ${chainConfig.NFT_ADDRESS}`
    );
    console.log(
      '\x1b[32m%s\x1b[0m',
      `View in block explorer: ${chainConfig.BLOCK_EXPLORER_URL}/address/${chainConfig.NFT_ADDRESS}`
    );
  });

task('deploy:marketplace', 'Deploys the Marketplace.sol contract')
  .addParam('chainId', 'The Chain ID of the blockchain where this contract will be deployed.')
  .setAction(async function (taskArguments, hre) {
    const availableChains = getAvailableChains();
    const { chainId } = taskArguments;
    ...
    let NFTMarketplace;
    if (account) {
      NFTMarketplace = await hre.ethers.getContractFactory('Marketplace', account);
    } else {
      NFTMarketplace = await hre.ethers.getContractFactory('Marketplace');
    }
    ...
    availableChains[chainId].NFT_MARKETPLACE_ADDRESS = nftMarketplace.address.toLowerCase();
    fs.writeFileSync(chainConfigFilePath, JSON.stringify(availableChains, null, 4));
...
task('deploy', 'Deploys the Marketplace.sol and NFT.sol contract')
  .addParam('chainId', 'The Chain ID of the blockchain where this contract will be deployed.')
  .setAction(async (taskArgs, hre) => {
    await hre.run('deploy:marketplace', taskArgs);
